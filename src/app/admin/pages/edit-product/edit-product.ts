...
export class EditProduct {
  ...
  loadProduct() {
    this.productService.getById(this.id).subscribe({
      next: (res: any) => {
        ...
        // Load existing images (up to 5)
        this.existingImages = [];
        if (p.pic1) this.existingImages.push(p.pic1);
        if (p.pic2) this.existingImages.push(p.pic2);
        if (p.pic3) this.existingImages.push(p.pic3);
        if (p.pic4) this.existingImages.push(p.pic4);
        if (p.pic5) this.existingImages.push(p.pic5);
        this.existingVideo = p.video || '';
      },
      error: (err) => console.error(err),
    });
  }
  ...
  onUpdate() {
    ...
    // Append new images if selected (up to 5)
